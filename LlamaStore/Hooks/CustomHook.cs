...
public class CustomHook : IHook
{
    ...
    {
        Console.WriteLine(
            $"Before request on URL {request?.RequestUri?.AbsoluteUri} with method {request?.Method.ToString().ToUpper()}"
        );
        return request;
    }
    ...
    {
        Console.WriteLine(
            $"After response on URL {response?.RequestMessage?.RequestUri?.AbsoluteUri} with method {response?.RequestMessage?.Method.ToString().ToUpper()}, returning status {response?.StatusCode}"
        );
        return response;
    }
